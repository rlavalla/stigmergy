...
class BluePlane:
    ...
    def move(self):
        self.position = (
            self.position[0] + random.uniform(-1000, 1000),
            self.position[1] + random.uniform(-1000, 1000),
            self.position[2] + random.uniform(-1000, 1000)
        )
...
def probability_of_guide(plane, weapon, target):
    ...
    print(f"PG Calculation for Plane {plane.id} with weapon range {weapon.range} and Target {target.id}: Distance = {distance:.2f}, Kinematics = {weapon.kinematics}, Expiring Factor = {weapon.expiring_factor}, Fuel = {plane.fuel}, PG = {pg:.4f}")
    return pg
    ...
    for plane in blue_planes:
        for weapon in plane.weapons:
            ...
            if pg > best_pg:
                ...
                best_weapon = weapon
                best_plane = plane
    if best_pg > 0 and best_plane and best_weapon:  # Ensure PG is greater than 0
        ...
        for target in targets[:]:
            ...
            if best_plane and best_weapon:
                if best_plane.fire_weapon(best_weapon):
                    ...
                    time.sleep(random.uniform(1, 10))  # Delay between target being shot down
                    break  # Recalculate after each target is shot down
            else:
                print(f"Plane {best_plane.id} is unable to fire at Target {target.id} due to PG being 0 or insufficient weapons")
                ...
                for remaining_target in targets:
                    reporting_sensors = get_reporting_sensors(remaining_target, reports)
                    if len(reporting_sensors) > 2:
                        best_pair = select_best_pair(reporting_sensors, remaining_target)
                        for sensor in reporting_sensors:
                            if sensor not in best_pair:
                                current_report = (sensor, remaining_target)
                                new_report = (best_pair[0], remaining_target) if best_pair[0] != sensor else (best_pair[1], remaining_target)
                                ensure_make_before_break_handoff(current_report, new_report)
                                reports = [(s, t) for s, t in reports if (s, t) != (sensor, remaining_target)]
                                print(f"Plane {sensor.id} stops reporting Target {remaining_target.id} to reduce redundancy")
